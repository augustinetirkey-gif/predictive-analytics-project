...
st.subheader("Project: Forecasting Trends & Business Outcomes (USD $)")
st.write("---")
...
with tab1:
    ...
    with c1:
        ...
        fig_prod = px.bar(df.groupby('PRODUCTLINE')['SALES'].sum().reset_index(), 
                          x='PRODUCTLINE', y='SALES', color='SALES', 
...
with tab2:
    ...
    with col_a:
        st.write("**Target Variable:** `SALES` (USD)")
...
with tab3:
    ...
    st.success("Random Forest Regressor trained successfully on 80% of historical data.")

# --- WEEK 5: EVALUATION & OPTIMIZATION ---
with tab4:
    st.header("📉 Performance Metrics (USD)")
