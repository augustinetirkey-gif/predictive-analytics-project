...
if uploaded_file is not None:
    ...
    # New Product Line Filter
    st_products = st.sidebar.multiselect("Product Lines", options=sorted(df_master['PRODUCTLINE'].unique()), default=df_master['PRODUCTLINE'].unique())
    
    # Apply Filters
    df = df_master[
        (df_master['YEAR'].isin(st_year)) & 
        (df_master['COUNTRY'].isin(st_country)) &
        (df_master['PRODUCTLINE'].isin(st_products))
    ]
    ...
    if df.empty:
        ...
        with tabs[0]:
            ...
            # --- REVENUE ANALYSIS SECTION (NEW) ---
            st.markdown("---")
            st.markdown("### 💰 Revenue Insight Matrix")
            r_col1, r_col2 = st.columns(2)
            
            with r_col1:
                st.markdown("#### Revenue by Country (Top Performing)")
                country_rev = df.groupby('COUNTRY')['SALES'].sum().reset_index().sort_values('SALES', ascending=False)
                fig_rev_bar = px.bar(country_rev, x='SALES', y='COUNTRY', orientation='h', 
                                    text_auto='.2s', color='SALES', color_continuous_scale='Greens', 
                                    template="plotly_white")
                st.plotly_chart(fig_rev_bar, use_container_width=True)
                
            with r_col2:
                st.markdown("#### Revenue Contribution by Deal Size")
                deal_rev = df.groupby('DEALSIZE')['SALES'].sum().reset_index()
                fig_deal_rev = px.funnel(deal_rev, x='SALES', y='DEALSIZE', color='DEALSIZE',
                                        template="plotly_white")
                st.plotly_chart(fig_deal_rev, use_container_width=True)
