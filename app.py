...
if uploaded_file is not None:
    ...
    with tabs[1]:
        ...
        max_qty_hist = int(df_master['QUANTITYORDERED'].max())
        
        st.info(f"💡 *Historical Context:* Avg Price: ${avg_msrp:.2f} | Max Hist Qty: {max_qty_hist}")

        # --- UNLOCKED INPUTS FOR EXTRAPOLATION ---
        # Increased max to 10,000 and allowed manual typing via number_input if preferred
        in_qty = col1.number_input("Quantity to Sell (Projected)", min_value=1, max_value=100000, value=50) 
        
        # Unlocked Price to allow testing luxury pricing or massive discounts
        in_msrp = col2.number_input("Unit Price ($)", min_value=0.01, max_value=100000.0, value=float(avg_msrp), step=0.01, format="%.2f")
        ...
        if st.button("RUN AI SIMULATION & REALITY CHECK", use_container_width=True, type="primary"):
            ...
            st.markdown(f"""
                <div style='background-color:#e3f2fd;padding:30px;border-radius:15px;text-align:center;border: 2px solid #1f4e79;margin-bottom:25px;'>
                    ...
                    <p style='font-size:12px; color:#555;'>*Based on {in_qty} units at ${in_msrp:.2f} each</p>
                </div>
            """, unsafe_allow_html=True)
            ...
            if not history.empty:
                ...
                st.write(f"**MAE:** {mae:,.2f} | **RMSE:** {rmse:,.2f} | **R² Score:** {r2*100:.2f}%")
